...
#pragma region Difined varaiables
...
typedef enum { META_COMMAND_SUCCESS, META_COMMAND_UNRECONGNIZED_COMMAND } MetaCommandResult;
...
PrepareResult prepare_statement(InputBuffer* input_buffer, Statement* statement) {
    if (strncmp(input_buffer->buffer, "insert", 6) == 0) {
        ...
        int args_assigned = sscanf(input_buffer, "insert %d %s %s", &(statement)->row_to_insert.id),
...
void execute_statement(Statement* statement) {
    ...
    case(STATEMENT_INSERT):
        printf("We insert now!");
        break;
    case(STATEMENT_SELECT):
        printf("We select now!");
        break;
    }
}
